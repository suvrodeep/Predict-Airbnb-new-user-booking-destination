...
##Uncomment packages needed to be newly installed
##
...
##
df.bar <-
  data.frame(Destination = unique(df_train$country_destination))

for (i in 1:nrow(df.bar)) {
  df.bar$Count[i] <-
    sum(df_train$country_destination == df.bar$Destination[i])
}

ggplot(data = df.bar,
       mapping = aes(
         x = df.bar$Destination,
         y = (df.bar$Count / sum(df.bar$Count) * 100)
       )) +
  xlab("Destination") + ylab("Percentage of Total") +  geom_col()
...
##
for (i in 1:nrow(df.err.multinom)) {
...
df.bar1 <-
  data.frame(Destination = unique(df.err.multinom$country_destination),
             Type = "Actual")
for (i in 1:nrow(df.bar1)) {
  df.bar1$Count[i] <-
    sum(df.err.multinom$country_destination == df.bar1$Destination[i])
}
df.bar2 <-
  data.frame(Destination = unique(df.err.multinom$country_destination),
             Type = "Predicted")
for (i in 1:nrow(df.bar2)) {
  df.bar2$Count[i] <-
    sum(df.err.multinom$dest_pred == df.bar2$Destination[i])
}

df.bar <- rbind(df.bar1, df.bar2)

graph.object <-
  ggplot(data = df.bar,
         mapping = aes(x = Destination, y = Count, fill = Type))
graph.object + geom_bar(stat = "identity", position = "dodge")
...
for (i in 1:nrow(df.bar1)) {
  df.bar1$Count[i] <-
    sum(df.error.calc1$country_destination == df.bar1$Destination[i])
}
df.bar2 <-
  data.frame(Destination = unique(df.error.calc1$country_destination),
             Type = "Predicted")
for (i in 1:nrow(df.bar2)) {
  df.bar2$Count[i] <-
    sum(df.error.calc1$dest_pred == df.bar2$Destination[i])
}
...
for (i in 1:nrow(df.bar1)) {
  df.bar1$Count[i] <-
    sum(df.error$country_destination == df.bar1$Destination[i])
}
df.bar2 <-
  data.frame(Destination = unique(df.error$country_destination),
             Type = "Predicted")
for (i in 1:nrow(df.bar2)) {
  df.bar2$Count[i] <-
    sum(df.error$dest_pred == df.bar2$Destination[i])
}
